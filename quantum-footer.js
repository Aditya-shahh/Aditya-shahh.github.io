...
// Full v6 simulation adapted for light theme footer
let qShow2=true, qShow3=true, qShow4=true, qShowPhase=false, qShowEntangle=false, qPinned=-1;
function qToggle(k){
  if(k===2)qShow2=!qShow2;if(k===3)qShow3=!qShow3;if(k===4)qShow4=!qShow4;
  document.getElementById('qBtn'+k).classList.toggle('active');
}
function qTogglePhase(){qShowPhase=!qShowPhase;document.getElementById('qBtnP').classList.toggle('active');}
function qToggleEntangle(){qShowEntangle=!qShowEntangle;document.getElementById('qBtnE').classList.toggle('active');}
function qPerturb(){
  for(let i=0;i<qN;i++){qPsi[i].re+=(Math.random()-0.5)*0.5;qPsi[i].im+=(Math.random()-0.5)*0.5;}
...
(function(){
  ...
  canvas.addEventListener('mousemove',e=>{const r=canvas.getBoundingClientRect();mx=e.clientX-r.left;my=e.clientY-r.top;mouseIn=true;});
  ...
  canvas.addEventListener('click',e=>{
    ...
    for(let i=0;i<qN;i++){const dx=toks[i].sx-clickX,dy=toks[i].sy-clickY;const d=Math.sqrt(dx*dx+dy*dy);if(d<25&&d<minD){minD=d;minI=i;}}
    qPinned=qPinned===minI?-1:minI;
  });

  // Quantum state
  const qN=window.qN=20;const EMB=8;
  const qPsi=window.qPsi=[];
  let ns0=0;
  ...
  function evolve(dt){
    ...
    for(let i=0;i<qN;i++){let hR=0,hI=0;
  ...
  for(let i=0;i<qN;i++){
    const phi=Math.acos(1-2*(i+0.5)/qN);const theta=Math.PI*(1+Math.sqrt(5))*i;
  ...
  function proj(bx,by,bz,t){
    let x=bx,y=by,z=bz;
    const ca=Math.cos(t*0.1),sa=Math.sin(t*0.1);let nx=x*ca+z*sa,nz=-x*sa+z*ca;x=nx;z=nz;
    const tilt=0.3+(mouseIn?(my-cy)/H*0.3:0);const ct=Math.cos(tilt),st=Math.sin(tilt);
    let ny=y*ct-z*st;nz=y*st+z*ct;y=ny;z=nz;
    if(mouseIn){const mr=(mx-cx)/W*0.4;const cm=Math.cos(mr),sm=Math.sin(mr);nx=x*cm+z*sm;nz=-x*sm+z*cm;x=nx;z=nz;}
    const fov=500;const p=fov/(fov+z);return{sx:cx+x*p,sy:cy+y*p,z,scale:p};
  }
  ...
  function drawMatrix(){
    if(!matCtx)return;
    const s=200,cell=s/qN;matCtx.fillStyle='#EBE5DF';matCtx.fillRect(0,0,s,s);
    for(let i=0;i<qN;i++)for(let j=0;j<qN;j++){
      const sc=a2(i,j);const intensity=Math.min(1,sc*2);
      if(qShowPhase){const ph=qPh(i)-qPh(j);const hue=((ph/Math.PI)*180+360)%360;matCtx.fillStyle=hsl(hue,50,40+intensity*30,intensity);}
      else{const r=Math.floor(intensity*179),g=Math.floor(intensity*75),b=Math.floor(intensity*54);matCtx.fillStyle=`rgba(${r},${g},${b},${0.1+intensity*0.9})`;}
      matCtx.fillRect(j*cell,i*cell,cell-0.3,cell-0.3);
    }
    if(qPinned>=0){matCtx.strokeStyle='rgba(179,75,54,0.7)';matCtx.lineWidth=1;matCtx.strokeRect(0,qPinned*cell,s,cell);matCtx.strokeRect(qPinned*cell,0,cell,s);}
  }
  ...
  function frame(){
    ...
    if(mouseIn){decoF=Math.min(1,decoF+0.02);let minD=Infinity,minI=0;
    ...
    for(let i=0;i<qN;i++){const tk=toks[i];
      const wx=Math.sin(t*0.35+tk.ph)*8,wy=Math.cos(t*0.3+tk.ph*1.3)*7,wz=Math.sin(t*0.25+tk.ph*0.7)*7;
      const p=proj(tk.bx+wx,tk.by+wy,tk.bz+wz,t);tk.sx=p.sx;tk.sy=p.sy;tk.z=p.z;tk.scale=p.scale;}
    ...
    // 4-body
    if(qShow4)for(const c of b4){const a=toks[c.i],b_=toks[c.j],ck=toks[c.k],d=toks[c.l];const al=c.s*0.12;
      ...
      const gx=(a.sx+b_.sx+ck.sx+d.sx)/4,gy=(a.sy+b_.sy+ck.sy+d.sy)/4;const gr=ctx.createRadialGradient(gx,gy,0,gx,gy,14);gr.addColorStop(0,`rgba(80,110,160,${al})`);gr.addColorStop(1,'rgba(80,110,160,0)');ctx.fillStyle=gr;ctx.beginPath();ctx.arc(gx,gy,14,0,Math.PI*2);ctx.fill();}

    // 3-body
    if(qShow3)for(const c of b3){const a=toks[c.i],b_=toks[c.j],ck=toks[c.k];const al=c.s*0.18;
      ctx.beginPath();ctx.moveTo(a.sx,a.sy);ctx.lineTo(b_.sx,b_.sy);ctx.lineTo(ck.sx,ck.sy);ctx.closePath();ctx.fillStyle=`rgba(179,75,54,${al*0.3})`;ctx.fill();ctx.strokeStyle=`rgba(179,75,54,${al*0.6})`;ctx.lineWidth=0.5;ctx.stroke();
      const gx=(a.sx+b_.sx+ck.sx)/3,gy=(a.sy+b_.sy+ck.sy)/3;const gr=ctx.createRadialGradient(gx,gy,0,gx,gy,8);gr.addColorStop(0,`rgba(179,75,54,${al})`);gr.addColorStop(1,'rgba(179,75,54,0)');ctx.fillStyle=gr;ctx.beginPath();ctx.arc(gx,gy,8,0,Math.PI*2);ctx.fill();}

    // 2-body
    if(qShow2)for(const c of b2){const a=toks[c.i],b_=toks[c.j];const al=c.s*0.4*Math.min(a.scale,b_.scale);
      const midX=(a.sx+b_.sx)/2,midY=(a.sy+b_.sy)/2;const dc=(a.z-b_.z)*0.003;const dx=b_.sx-a.sx,dy=b_.sy-a.sy;
      ctx.beginPath();ctx.moveTo(a.sx,a.sy);ctx.quadraticCurveTo(midX-dy*dc,midY+dx*dc,b_.sx,b_.sy);ctx.strokeStyle=`rgba(90,86,80,${al})`;ctx.lineWidth=0.4+c.s*1.2;ctx.stroke();}

    // Entanglement
    if(qShowEntangle)for(let i=0;i<qN;i++)for(let j=i+1;j<qN;j++){const ent=Math.abs(qPsi[i].re*qPsi[j].re+qPsi[i].im*qPsi[j].im);
      if(ent>0.05){ctx.beginPath();ctx.moveTo(toks[i].sx,toks[i].sy);ctx.lineTo(toks[j].sx,toks[j].sy);ctx.strokeStyle=`rgba(139,80,168,${ent*0.5})`;ctx.lineWidth=ent*2;ctx.setLineDash([2,4]);ctx.stroke();ctx.setLineDash([]);}}

    // Pinned
    if(qPinned>=0)for(let j=0;j<qN;j++){if(j===qPinned)continue;const s=a2(qPinned,j);if(s>0.1){ctx.beginPath();ctx.moveTo(toks[qPinned].sx,toks[qPinned].sy);ctx.lineTo(toks[j].sx,toks[j].sy);ctx.strokeStyle=`rgba(179,75,54,${s*0.6})`;ctx.lineWidth=s*2.5;ctx.stroke();}}

    // Phase fringes
    if(qShowPhase){ctx.globalAlpha=0.35;
      for(let x=0;x<W;x+=8)for(let y=0;y<H;y+=8){let rS=0,iS=0;
        ...
        const inten=rS*rS+iS*iS;if(inten>0.01){ctx.beginPath();ctx.arc(x,y,Math.min(3,inten*4),0,Math.PI*2);ctx.fillStyle=`rgba(90,86,80,${Math.min(0.25,inten*0.4)})`;ctx.fill();}}
      ctx.globalAlpha=1;}

    // Observer
    if(mouseIn){for(let i=0;i<qN;i++){const dx=toks[i].sx-mx,dy=toks[i].sy-my;const dist=Math.sqrt(dx*dx+dy*dy);
        if(dist<200){const a=0.2*(1-dist/200)*bP(i)*3;ctx.beginPath();ctx.moveTo(mx,my);ctx.quadraticCurveTo((mx+toks[i].sx)/2-dy*0.03,(my+toks[i].sy)/2+dx*0.03,toks[i].sx,toks[i].sy);ctx.strokeStyle=`rgba(179,75,54,${a})`;ctx.lineWidth=0.5+a*2;ctx.stroke();}}
    ...
    for(const{tk,i}of sorted){const prob=bP(i);const ph=qPh(i);const bSz=2.5+tk.scale*3.5;const sz=bSz*(0.5+prob*3);const alpha=0.2+tk.scale*0.6;const phaseHue=((ph/Math.PI)*180+360)%360;
      if(prob>0.02){const cr=sz*(2+prob*12);const gr=ctx.createRadialGradient(tk.sx,tk.sy,0,tk.sx,tk.sy,cr);
      ...
      ctx.beginPath();ctx.arc(tk.sx,tk.sy,sz,0,Math.PI*2);ctx.fillStyle=qShowPhase?hsl(phaseHue,60,45,alpha):`rgba(90,86,80,${alpha})`;ctx.fill();
      ...
      ctx.beginPath();ctx.arc(tk.sx-sz*0.2,tk.sy-sz*0.2,sz*0.25,0,Math.PI*2);ctx.fillStyle=`rgba(235,229,223,${alpha*0.3})`;ctx.fill();}
    ...
    if(lbl){if(decoF>0.7){lbl.textContent='COLLAPSED';lbl.style.borderColor='#B34B36';lbl.style.color='#B34B36';}
    ...
    if(mi){if(qPinned>=0)mi.textContent=`pinned: token ${qPinned} · P(${qPinned}) = ${bP(qPinned).toFixed(4)} · φ = ${qPh(qPinned).toFixed(3)}`;
